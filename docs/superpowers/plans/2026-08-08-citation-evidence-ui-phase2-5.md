...
>
> この検証で**自動テストでは検出できない欠陥を1件発見**した(修正済み):
> 日本語の quote は改行を空白へ畳むと PDF 本文と一致せず、**矩形が常に空**だった。
> `search_for` は行をまたぐ日本語文字列をまず見つけられない(実測: 連結すると0件、
> quote の元の改行で割ると4行すべてヒット)。行単位の部分一致を1段目に置いて解決。
