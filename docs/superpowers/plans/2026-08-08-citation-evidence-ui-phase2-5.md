...
## ゲート実測結果 (2026-08-08 実施)

隔離環境(`NOTEBOOK_OLLAMA_DATA_DIR=./.gate-data` / API `:8801` / vite `:5198`、
Playwright は FIFO ロック経由)。本番(`:8765`・既定 data_dir)には非接触。

> [!success] Phase 2 ゲート — PASS
> - 原本タブが実データで描画され、`原本 p.6` のラベル・ページ送り(◀p.5 / 拡大 / p.7▶)が動作
> - 引用を切り替えるとタブ表記とページが追随する
> - **根拠箇所の矩形が該当ページの表内に描画された**(スクショ `.gate-shots/p2-03-rects.png`)
>
> **訂正 (2026-08-08)**: このゲートで「日本語の矩形が常に空」と報告し `3913a13` を
> 入れたが、**これは診断の誤りだった**。API を叩く際に p.6 のチャンクの quote へ
> p.11 を組み合わせて投げており、ページ違いで当たらなかっただけである。
> 修正前のロジック(空白へ畳んで `search_for`)でも、正しいページでは 10 個の矩形が
> 取れていた(実測: 修正前ロジック p6=10 / p11=0)。
> `3913a13` で足した空白除去と行単位フォールバックは無害で他のケースでは効きうるが、
> コミットメッセージに書いた根本原因の説明は誤りである。
>
> なお1つの根拠に矩形が複数出るのは正常。`search_for` は一致が複数行にまたがると
> 行ごとに矩形を返すため、10 個の枠は「10箇所」ではなく「1箇所が10行に折り返して
> いる」という意味。

> [!success] Phase 5 ゲート — PASS
> - テキストタブで選択すると「訳」ボタンが選択箇所の隣に浮く(`p5-01-translate-fab.png`)
> - 押すと「翻訳中…」→ 訳文が**本文の下にインラインで差し込まれ、原文は残る**
>   (`p5-03-translation-inline.png`)
> - モデルは `qwen3:30b-a3b`。初回はモデルロードのぶん待ちが入る
>
> **未実施**: 翻訳専用モデルを別指定したときの切替待ちの計測(Task 14 Step 5)。

> [!success] Phase 3 (quote モードβ) ゲート — 言語跨ぎで 0件 → 9件
> 英語ソース(`.gate-shots/en-capability.pdf`)に日本語で同じ3問を、OFF と ON で
> それぞれ新しい会話に投げて比較した(`qwen3:30b-a3b`)。
>
...
> | ON | **9** | 4 | 5 | 40.4 | 296 |
>
> - **言語跨ぎで根拠が示せるようになった**(0件 → 9件)。設計書 §2 の適用限界を
>   ON で越えられることを実測で確認
> - レイテンシ **+40%**、出力長 **+123%**。設定画面の「応答が遅くなります」は実態と合う
> - `<q>` タグの表示漏れ **0/3**(`strip_quote_tags` が効いている)
> - **quote が本文と一致したのは 9件中4件(44%)**。残り5件は第1段(字句照合)が補完した。
>   quote モードは回答に原文を写させるため、結果として字句照合も当たりやすくなる
>   副次効果がある(出現単位のマージが効いている証拠でもある)
>
> 計測後、フラグは既定(OFF)に戻してある。
