...
master マージ後、「タイル分割と視覚検索が実際に効くか」を実測した。**コードは1行も
変更していない。**詳細な生ログ・4条件×6問の生データは
`.superpowers/sdd/stage4-experiment/measurement-log.md` を参照(スクリプト・生JSONは
コントローラのスクラッチパッドに残置)。

### 実験条件

`AN220191_DMA_TRAVEO_T2G_JA.pdf`(TRAVEO T2G DMAアプリケーションノート、日本語機械翻訳
版、167ページ)を隔離データディレクトリ・ポート8766に取り込み、テキスト埋め込み
`bge-m3`(Ollama, CPU)+ 視覚埋め込み `Qwen/Qwen3-VL-Embedding-2B`(transformers, CUDA,
RTX 2080 Ti)でpage/tile両索引を構築した。

### 発見: `build_cooldown_seconds` の既定値がGPUでも無条件に効く

§8.3 は「`build_cooldown_seconds` / `cpu_threads` / P-core分業は `device == "cpu"` 分岐の
内側なので GPU なら自動的に無効化される」と書いているが、実装(`core/visual/
index_builder.py` のページループ)では **`build_cooldown_seconds` だけがデバイスを見ずに
無条件でページ間に挟まる**。既定値10.0のままGPUで構築したところ、1ページの埋め込み
自体は0.4秒程度で終わるにもかかわらず170秒経過してもpage索引(167ページ)の構築が
終わらず(コントローラが並行して「30秒で4ページ」=約7.5秒/ページのオーダーの遅さを
...
誤った値になる。上記の73秒/96秒は自分でPOST発行時刻を記録した一次計測値。)

### 4条件×6問のretrieval測定(生成は測っていない)

`RetrievalService` を直接構築し、`VisualSearchDeps` の `unit_getter` / `strategy_getter`
のみを差し替えて4条件(page/tile × hybrid_rrf/visual_only)を測定した。質問は表2問・
図2問・本文2問、正解ページを事前に特定した上で `top_k=8` の順位を記録した。

**主指標(6問中、top-3 / top-5 ヒット数)**:

| 条件 | top-3 | top-5 |
|---|---|---|
| A: page / hybrid_rrf(現行既定) | 5/6 | 5/6 |
| B: tile / hybrid_rrf | 5/6 | 5/6 |
| C: page / visual_only | 4/6 | 6/6 |
| D: tile / visual_only | 5/6 | 6/6 |

**所見(正直な記録。差が出なかった/効果を確認できなかった点も含む)**:

- **A条件とBの条件(page vs tile、hybrid_rrf戦略)は6問全てで順位が完全一致した。**
  RRF融合ではテキスト側が常に視覚側より上位に来ており、hybrid_rrf戦略下では
  `index_unit`(page/tile)の選択が最終結果に一切影響しなかった。
- **タイル分割の効果は一様ではなかった**: visual_only戦略内でのpage vs tile比較では
  6問中2問改善・1問悪化(ページ全体で完結する図の質問がタイル分割で順位悪化)・
  3問同着。「タイル分割が一貫して検索を助ける」という結果にはならなかった。
- **視覚のみ(visual_only)がhybrid_rrfより良い結果になった質問があった**
  (表に関する質問1問がhybrid_rrfではtop_k圏内ギリギリ、visual_onlyでは明確に浮上)。
  一方でhybrid_rrf条件では正解ヒットが6問全てテキスト側から来ており、視覚検索が
  テキストの見逃しを救ったケースは今回ゼロだった。
- **サンプル数の限界**: 6問・1文書のみの測定であり、統計的な有意差は主張できない。
  また実験に使ったPDFは機械翻訳PDFで図のラベル・説明文がテキストとして抽出可能な
  ため、「図」種別の質問も実際にはテキスト検索で解けてしまっていた(hybrid_rrf条件の
  図質問の正解ヒットは両方ともテキスト側経由)。スキャン画像PDFで再現していれば
  結果が変わった可能性が高い。

**タイル既定値(rows=3, cols=1, overlap=0.1)を変えるべきかの判断**: 変更を推奨する
根拠は今回のデータからは得られなかった。効果が質問依存で一様でなく(改善/悪化/同着が
混在)、サンプル数(6問・1文書)も既定値変更の意思決定には不十分。**現状維持が妥当。**
follow-upとして、より多様な文書(スキャン画像PDF・2段組・図が単独ページを占める資料)
での再測定を提案する。

**生成(LLM)は測っていない**: LLM出力はサンプリングでブレるうえ、24回のチャット
応答待ちは所要時間が大きい。またretrievalの精度という問いに生成側の要因(プロンプト
構成・モデル差)を混ぜると測定が濁るため、実験設計の段階で主指標を「retrieval正解
ページの順位」に絞った。`pixel_native`戦略も当初定義した4条件に含まれておらず測定
対象外(vision対応モデル `gemma3:12b` は環境にあるため追加実験自体は可能)。
