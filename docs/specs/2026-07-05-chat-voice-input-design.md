...
### やること
- ChatInput へのマイクボタン追加(モード別の録音制御、認識テキストを textarea のカーソル位置に挿入)
...
| 4 | 認識結果の扱い | textarea のカーソル位置に挿入のみ(自動送信しない) | 固有名詞・専門用語の誤認識を送信前に修正できる。Cmd/Ctrl+Enter で即送信可能 |
| 5 | PTT キー | 既定 Space、設定画面で単一キー(`KeyboardEvent.code`)に変更可能 | Claude Code の慣行に合わせる |
| 5b | タップと長押しの判別 | タップ(しきい値未満で解放)= 通常のキー入力、長押し(しきい値超え)= PTT 録音。**textarea フォーカス中でも動作** | Claude Code と同じく「押し方」でユーザー意図を汲む。ブラウザは keydown/keyup + repeat フラグが取れるため、ターミナル(キーリピート連打検出+ウォームアップ文字の事後削除)より素直に実装できる |
...
[ChatInput / document キーフック]              [FastAPI]
 PTTキー keydown(repeat は無視、IME 変換中は不介入)
   → preventDefault + 長押し判定タイマー開始(しきい値 250ms、定数で調整可)
 ├ しきい値未満で keyup(タップ)
 │   → 何も開始していないので破棄処理は不要。キーの通常文字を 1 打鍵分だけ
 │     カーソル位置に挿入(Space なら空白 1 個。textarea フォーカス中でも通常入力が成立)
 └ しきい値超えて保持(長押し確定、250ms)= PTT 録音中
     → この時点で音声キャプチャを開始する(keydown 先行開始は不採用。理由は下記)
     キー解放
       → クライアントで WAV 化し multipart POST ─→ POST /api/stt/transcribe
                                                    ...
                                                    └ セグメント text 連結
     ← {text, duration_ms} ←───────────────────────┘
       → textarea の**カーソル位置に挿入**(Claude Code と同じ。カーソルは挿入末尾へ)
```

- タップ = 通常入力 / 長押し = 録音 の判別により、**textarea にフォーカスがあるままでも
  PTT が使える**(Claude Code の hold モードと同じ操作感)
- IME 変換中(`KeyboardEvent.isComposing`)は一切介入しない(Space は変換候補選択のまま)
- 修飾キー(Ctrl/Alt/Meta/Shift)同時押しは PTT 対象外(既存ショートカットを妨げない)
- マイクボタンの**長押し**(mouse/touch)でも同じ動作
- 120 秒に達したら自動停止して送信(クライアント側で実施。サーバーは長さチェックしない)
- トレードオフ: PTT キーの押しっぱなしによるキーリピート入力(空白連打等)はできなくなる
  (Claude Code も同様)。必要なら設定でモードを「無効」にすれば通常入力に戻る
- **キャプチャ開始タイミング(改訂)**: キャプチャは長押し確定(250ms)時に開始する。
  keydown 先行開始は Space タップのたびに getUserMedia が走りマイクインジケーターが
  点滅する問題(通常タイピング時)があるため不採用。冒頭 ~数百ms のロスは Claude Code
  のホールドウォームアップ相当で許容する

### 常時有効(ハンズフリー)

```
 マイクボタンクリック(または PTT キー長押し)でオン/オフをトグル
   (キーのタップは通常入力のまま。長押しジェスチャの意味を PTT モードと揃える)
 オンの間:
   ブラウザ側 VAD(RMS しきい値 + ハングオーバー)が発話区間を検出
     - プリロール約 300ms / 無音約 800ms で区間確定 / 1 区間最大 30 秒で強制分割
   区間確定のたびに WAV 化 → POST /api/stt/transcribe → textarea のカーソル位置に挿入
   POST はクライアント側キューで直列化(応答順の入れ替わりによる挿入順序乱れを防ぐ)
```
...
- モード別挙動:
  - **PTT**: キー押下中またはボタン長押し中のみ録音中表示(赤・パルス + 経過秒)。
    解放で変換中(Spinner)→ カーソル位置に挿入
  - **ハンズフリー**: クリックでトグル。オン中は常時パルス表示 + 変換中は小 Spinner 併記
  - **無効**: ボタン非表示・キーフックなし
- PTT キーフックは document レベルで登録し、**タップと長押しで挙動を分ける**(§4):
  タップ(しきい値 250ms 未満)はキーの通常文字をカーソル位置に挿入、長押しは PTT 録音。
  textarea フォーカス中でもそのまま使える。`repeat` イベントは無視、IME 変換中
  (`isComposing`)と修飾キー同時押しは不介入
...
- **Space キーの競合**: タップ = 通常入力 / 長押し = PTT の判別(§4)で共存させる。
  keydown 時に `preventDefault()` するため、タップ時の文字はプログラム的に挿入する
  (undo 履歴を保つ実装方法は plan で確定。`execCommand('insertText')` 等)。
  IME 変換中は `isComposing` ガードで不介入(Space の変換候補選択を壊さない)。
  長押し判定しきい値(250ms)は定数として調整可能にする
