...
async def test_pdf_parser_emits_one_section_per_page():
    p = PdfParser()
    doc = await p.parse_bytes(FIXTURE.read_bytes(), source_hint="doc.pdf")
    pages = [s.page for s in doc.sections]
    assert pages == [1, 2]


async def test_pdf_parser_extracts_text_per_page():
    p = PdfParser()
    doc = await p.parse_bytes(FIXTURE.read_bytes())
