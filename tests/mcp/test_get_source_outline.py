...
from core.storage.sources_repo import create_source, update_source_status, SourceStatus
...
def test_outline_returns_title_pages_and_headings(tmp_path):
    ...
    src = create_source(conn, notebook_id=nb.id, kind="pdf",
                        origin="x.pdf", content_hash="h", title="Doc Title")
    ...
    chunks = [
        ChunkRecord(id="a"*26, source_id=src.id, notebook_id=nb.id, ord=0,
                    page=1, heading_path="Ch1", text="x", token_count=1),
        ChunkRecord(id="b"*26, source_id=src.id, notebook_id=nb.id, ord=1,
                    page=2, heading_path="Ch2", text="y", token_count=1),
    ]
