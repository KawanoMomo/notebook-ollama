    ...
    async def run(
        ...
        *,
        ...
        for idx, hit in enumerate(hits, start=1):
            location = format_location(page=hit.page, heading_path=hit.heading_path)
            prompt_chunks.append(PromptChunk(
                n=idx, title=hit.source_title, location=location, text=hit.text
            ))
